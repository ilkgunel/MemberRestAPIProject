...
public class MemberDeleteService extends BaseService {
    ...
    public MemberOperationPojo deleteBulkUserMember(List<MemberIdWrapp> memberIdList) {
        MemberOperationPojo deleteMemberOpertaionPojo = memberUtil.checkMemberExistenceOnMemberList(memberIdList, ConstantFields.ROLE_USER.getConstantField());
        ...
        if (ObjectUtils.isEmpty(deleteMemberOpertaionPojo.getErrorCode())) {
            ...
            memberOperationPojo.setResult(resourceBundleMessageManager.getValueOfProperty(ConstantFields.ROLE_USER + "_bulkMemberDeletingSuccessfull", deletedMemberList.get(0).getMemberLanguageCode()));

...
public class MemberSaveService extends BaseService {
	...
	public MemberOperationPojo addBulkMember(List<Member> memberList, String role) {
		...
		try {
			...
			memberOperationPojo.setResult(environment.getProperty(role + "_bulkMemberAddingSuccessfull") + memberList);
			LOGGER.log(Level.INFO, environment.getProperty(role + "_bulkMemberAddingSuccessfull") + memberList);
		} catch (CustomException e) {
			LOGGER.log(Level.SEVERE, environment.getProperty(role + "_bulkMemberAddingFaled") + e.getErrorCode() + " "
					+ e.getErrorMessage());
			memberOperationPojo.setErrorCode(e.getErrorCode());
			memberOperationPojo.setResult(e.getErrorMessage());
		} catch (Exception e) {
